...
def isInsideBounds(coordinates, bounds):
...
def allMatches(data, templates, threshold = 0.8):
	...
	for template in templates:
		...
		result = cv2.matchTemplate(data_gray, template_gray, cv2.TM_CCOEFF_NORMED)	
		loc = numpy.where(result >= threshold)
		points = zip(*loc[::-1])
		rects = [(point[0], point[1], point[0] + w, point[1] + h) for point in points]

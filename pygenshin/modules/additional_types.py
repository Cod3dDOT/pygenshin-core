class Vector2:
    ...
    def __truediv__(self, other):
        if isinstance (other, Vector2):
            if (other.x == 0): raise ValueError("x: Can not divide by 0")
            if (other.y == 0): raise ValueError("y: Can not divide by 0")
            return Vector2(self.x / other.x, self.y / other.x)

        if isinstance(other, int) or isinstance(other, float):
            if (other == 0): raise ValueError("other: Can not divide by 0")
...
class Rect:
    ...
    def __init__(self, startLocation: Vector2, endLocation: Vector2) -> None:
        ...
        self.mss = {"left": self.start.x, "top": self.start.y, "width": self.end.x - self.start.x, "height": self.end.y - self.start.y}

    @classmethod
    def fromTuples(cls, startLocation=(0, 0), endLocation=(0, 0)):
       return cls(Vector2(startLocation), Vector2(endLocation))

    @classmethod
    def fromRect(cls, rect=(0, 0, 0, 0)):
